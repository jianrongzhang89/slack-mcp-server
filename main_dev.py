#!/usr/bin/env python3
"""
MCP server for development with `mcp dev` command.
Uses the standard MCP library instead of fastMCP.
Includes Slack integration for querying messages and channels.
"""
...
@mcp.tool()
def ping_test() -> str:
    """Simple ping test to verify MCP connection is working."""
    import time
    start_time = time.time()
    time.sleep(0.1)  # Small delay to simulate work
    end_time = time.time()
    return f"Ping successful! Response time: {(end_time - start_time)*1000:.1f}ms"
